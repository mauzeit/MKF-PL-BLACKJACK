...
const { buildIndex, matchPlayer, normalize } = require("./lib/match");

const managers = JSON.parse(
  fs.readFileSync(path.join(__dirname, "data", "managers.json"), "utf8")
);

// Manual goal corrections — for cases where FPL's data lags or disputes the
// official Premier League record (see data/adjustments.json). Matched by
// normalized player name against whichever FPL player a pick resolves to,
// so one entry covers that player everywhere they're picked.
const adjustmentsPath = path.join(__dirname, "data", "adjustments.json");
const adjustments = fs.existsSync(adjustmentsPath)
  ? JSON.parse(fs.readFileSync(adjustmentsPath, "utf8"))
  : [];

function adjustmentFor(player) {
  return adjustments.find(
    (a) => normalize(a.name) === player.n_web || normalize(a.name) === player.n_second
  );
}
