...
const mobileCards = document.getElementById("board-actual-mobile");
const actualView = document.getElementById("actual-view");
const paceView = document.getElementById("pace-view");
...
function statusFor(row) {
  ...
  const remaining = 4 - row.scorers;
  const label = remaining === 1 ? "NEEDS 1 SCORER" : `NEEDS ${remaining} SCORERS`;
  return { key: "needs", label };
}
...
function adjustFlagFor(pick) {
  if (!pick.adjusted) return "";
  return `<span class="flag adjusted" title="${pick.adjustNote || "Manually corrected"}">†</span>`;
}
...
function renderRow(row, i) {
  ...
  const picksHtml = row.picks
    .map((p) => {
      ...
      return `<span class="pick-chip ${zeroClass} ${nf} ${hit}" title="${p.reason || ""}">${label} <span class="g">${p.found ? p.goals : "—"}</span> ${flagFor(p)}${adjustFlagFor(p)}</span>`;
    })
    .join("");

  const toTxt = row.bust ? "—" : row.toTarget;
  const toClass = row.bust ? "bust" : row.toTarget === 0 ? "zero" : "";
  const managerLabel = row.leading
    ? `${row.manager} 👑`
    : row.bust
    ? `${row.manager} ☠️`
    : row.manager;
  ...
  tr.className = row.leading ? "leading" : row.bust ? "bust-row" : "";
  tr.innerHTML = `
    <td class="col-num rank">${i + 1}</td>
    <td class="col-manager manager-name">${managerLabel}</td>
...
function statusForCompact(row) {
  if (row.leading) return { key: "leading", label: "LEADING" };
  if (row.bust) return { key: "bust", label: "BUST" };
  if (row.allFourScored) return { key: "in", label: "STILL IN" };
  return { key: "needs", label: `NEEDS ${4 - row.scorers}` };
}

function renderCard(row, i) {
  const st = statusForCompact(row);
  const managerLabel = row.leading
    ? `${row.manager} 👑`
    : row.bust
    ? `${row.manager} ☠️`
  ...
  const subText = row.bust
    ? `BUST · ${row.scorers}/4 SCORED`
    : `${row.toTarget} TO 21 · ${row.scorers}/4 SCORED`;

  const picksHtml = row.picks
    .map((p) => {
      const zero = p.goals === 0 ? "zero" : "";
      const nf = !p.found ? "notfound" : "";
      const meta = p.found
        ? `${p.teamShort ? `<span>${p.teamShort}</span>` : ""}<span class="g">${p.goals}</span> ${flagFor(p)}${adjustFlagFor(p)}`
        : `<span class="g">—</span> ${flagFor(p)}`;
      const label = p.found ? p.name : `${p.name}?`;
      return `<div class="mgr-pick-row ${zero} ${nf}"><span class="pn">${label}</span><span class="pm">${meta}</span></div>`;
    })
    .join("");

  const card = document.createElement("div");
  card.className = `mgr-card ${row.bust ? "bust-row" : ""}`;

  const head = document.createElement("button");
  head.className = "mgr-card-head";
  head.setAttribute("aria-expanded", "false");
  head.innerHTML = `
    <span class="mgr-card-rank">${i + 1}</span>
    <span class="mgr-card-name">${managerLabel}</span>
    <span class="mgr-card-goals"><span class="n">${row.total}</span><span class="u">GLS</span></span>
    <span class="mgr-card-badge status-badge ${st.key}">${st.label}</span>
    <span class="mgr-card-chevron">▶</span>
  `;

  const sub = document.createElement("p");
  sub.className = "mgr-card-sub";
  sub.textContent = subText;

  const picksWrap = document.createElement("div");
  picksWrap.className = "mgr-card-picks hidden";
  picksWrap.innerHTML = picksHtml;

  head.addEventListener("click", () => {
    const expanded = card.classList.toggle("expanded");
    picksWrap.classList.toggle("hidden", !expanded);
...
function renderActual() {
  ...
  if (!filtered.length) {
    ...
    mobileCards.innerHTML = `<p class="loading">Nothing matches.</p>`;
    return;
  }
  filtered.forEach((row, i) => boardBody.appendChild(renderRow(row, i)));
  filtered.forEach((row, i) => mobileCards.appendChild(renderCard(row, i)));
}
...
function renderPace() {
  ...
  withDiff.sort((a, b) => a.absDiff - b.absDiff || b.r.scorers - a.r.scorers || b.r.total - a.r.total);
...
function openModal(row) {
  ...
  const adjusted = row.picks.filter((p) => p.adjusted);
  const adjustedLines = adjusted
    .map((p) => `<p><strong>† ${p.name}</strong> — ${p.adjustNote}</p>`)
    .join("");
